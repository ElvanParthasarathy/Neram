  ...
  useEffect(() => {
    if (isRep && repBatch && repDept) {
      ...
      if (path.batch !== repBatch || path.dept !== repDept || viewLevel === 'batches' || viewLevel === 'depts') {
        updateLevel('editor', { batch: repBatch, dept: repDept }, true);
      }
    }
            ...
            {item.batches.map((b, idx) => (
              <div key={b.id} className="exam-subject-row professional animate-slide-down" style={{ position: 'relative' }}>
                ...
                <div className="input-group-vertical variant-scope">
                  <label>Scope</label>
                  <select className="event-input" value={b.scope || 'Common'} onChange={e => updateBatchFieldLocal(b.id, 'scope', e.target.value)} disabled={isRep}>
                    {isRep ? (
        ...
        <div className="breadcrumb-nav">
          {isRep ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 600, color: 'var(--mac-text)', fontSize: '15px' }}>
              <RiComputerLine style={{ fontSize: '18px', color: 'var(--mac-accent)' }} />
            ...
            <div className="breadcrumb-list">
              <span className="crumb-btn level-root" onClick={() => updateLevel('batches', { batch: '', dept: '' }, true)}>Classes</span>
