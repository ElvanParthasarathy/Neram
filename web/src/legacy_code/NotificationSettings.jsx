...
const NotificationSettings = ({ onBack }) => {
    const { testTrigger } = useNotification();
    // Shared loader
    ...
    const [testTime, setTestTime] = useState("05:30");
    ...
    return (
        <>
            ...
            <div className="s2-section-label" style={{ color: 'var(--mac-traffic-yellow)' }}>Notification Tester (Dev Tools)</div>
            <SettingsGroup>
                <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(255, 159, 10, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--mac-traffic-yellow)' }}>
                            <RiBugLine size={20} />
                        </div>
                        <div style={{ flex: 1 }}>
                            <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--mac-text)', display: 'block', marginBottom: '4px' }}>Simulate Schedule Triggers</span>
                            <span style={{ fontSize: '13px', color: 'var(--mac-text-secondary)', lineHeight: 1.4, display: 'block' }}>See exactly what notifications the engine will dispatch for a specific date and time setting. Bypasses once-per-day limit.</span>
                        </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        <input
                            ...
                            style={{
                                flex: 1,
                                background: 'var(--mac-bg-primary)', border: '1px solid var(--mac-glass-border)', color: 'var(--mac-text)',
                                padding: '10px 14px', borderRadius: '10px', fontSize: '14px', outline: 'none', fontFamily: 'inherit'
                            }}
                        />
                        <input
                            type="time"
                        ...
                        />
                        <button
                            onClick={() => testTrigger(testDate, testTime)}
                            style={{
                                background: 'var(--mac-blue)', color: 'white', border: 'none',
                                padding: '11px 20px', borderRadius: '10px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
