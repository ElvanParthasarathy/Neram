export class NotificationLogic {
    ...
    checkTimeTriggers(mockDateStr = null, mockTimeStr = null, isTest = false) {
        const now = new Date();
        const currentHour = now.getHours().toString().padStart(2, '0');
        const currentMinute = now.getMinutes().toString().padStart(2, '0');

        const currentTime = mockTimeStr || `${currentHour}:${currentMinute}`;
        const todayDateStr = mockDateStr || now.toISOString().split('T')[0];
        ...
        if (currentTime === time1) {
            this.fireMorningWake(todayDateStr, dailyUpdate, generalNotice, examAlerts, isTest);
        } else if (currentTime === time2) {
            this.firePreCollege(todayDateStr, classSchedule, labReminders, isTest);
        } else if (currentTime === time3) {
            this.fireCollegeEntry(todayDateStr, isTest);
        } else if (isTest) {
            // Provide feedback if the tested time doesn't match any of the alarms
            this.dispatch("🛠️ Test Triggered", `However, ${currentTime} does not match any of your 3 configured alarm slots!`);
        }
    }
    ...
    fireMorningWake(dateStr, showDaily, showNotice, showExams, isTest = false) {
        if (!isTest && this.hasFired("morning", dateStr)) return;
        ...
        if (!isTest) this.markFired("morning", dateStr);
    }

    firePreCollege(dateStr, showSchedule, showLabs, isTest = false) {
        if (!isTest && this.hasFired("precollege", dateStr)) return;
        ...
        if (showLabs) {
            ...
            if (labs.length > 0) {
                ...
                if (isBatchLab || labs.some(l => !l.batch)) {
                    ...
                    if (!isTest) this.markFired("precollege", dateStr);
    ...
    fireCollegeEntry(dateStr, isTest = false) {
        if (!isTest && this.hasFired("entry", dateStr)) return;

        // This could be tied to location logic if added later, or just a simple reminder to scan ID

        if (!isTest) this.markFired("entry", dateStr);
    }
}
