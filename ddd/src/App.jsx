...
function App() {
  ...
  useEffect(() => {
      ...
      if (u) {
        ...
        userUnsubscribe = onValue(userRef, (snapshot) => {
          ...
          if (!userData.batch || !userData.department || !userData.section) {
            setShowForcedSetup(true);
          } else {
            setShowForcedSetup(false);
          }

          if (adminEmails.map(e => e.toLowerCase()).includes(u.email.toLowerCase())) {
            setIsAdminUser(true);
    ...
    const handleAdminViewChange = () => {
      const saved = sessionStorage.getItem("admin_preview_session");
      setActiveProfile(saved ? JSON.parse(saved) : null);
    };
    window.addEventListener('adminViewChanged', handleAdminViewChange);

    return () => {
      authUnsubscribe();
      if (userUnsubscribe) userUnsubscribe();
      window.removeEventListener('adminViewChanged', handleAdminViewChange);
    };
  }, []);

  useEffect(() => {
    // Determine which profile to use for fetching data
    const profileToUse = activeProfile || dbUserProfile;

    if (!profileToUse?.batch || !profileToUse?.section) return;
    const { batch, department, section } = profileToUse;
  ...
  }, [dbUserProfile, activeProfile, runGlobalCalendarSync]);
