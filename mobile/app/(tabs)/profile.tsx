...
export default function ProfileScreen() {
  ...
  const { data: profileData, loading: profileLoading } = useSyncData(
    `users/${uid}`,
    `STORAGE_PROFILE_${uid}`
  );

  // 2. Fetch Hierarchy (For Class Selection)
  const { data: hierarchy } = useSyncData(
    'academic_hierarchy',
    'STORAGE_HIERARCHY'
  );
  ...
  // Only show loader if we have NO data at all (first launch + no net)
        ...
        <View style={styles.avatarContainer}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (googlePhoto) {
                update(ref(db, `users/${user.uid}`), { photoURL: googlePhoto });
                setFormData({ ...formData, photoURL: googlePhoto }); // Optimistic
              }
            }}
          >
