...
class FirebaseRepository(private val context: Context) {
    ...
    private val googleCalendarService = com.elvan.rmdneram.data.remote.GoogleCalendarService()
    
    /**
     * Fetch calendar config from Firebase
     * Contains: apiKey, calendarId, semConfig (start, end dates)
     */
    suspend fun getCalendarConfig(batch: String): CalendarConfig? {
        return try {
            val ref = database.getReference("calendars/$batch")
            val snapshot = awaitSingleValue(ref)
            
            if (!snapshot.exists()) {
                Log.d(TAG, "No calendar config found for batch: $batch")
                return null
            }
            
            val config = snapshot.child("config")
            val semConfig = snapshot.child("semConfig")
            
            CalendarConfig(
                apiKey = config.child("apiKey").getValue<String>() ?: "",
                calendarId = config.child("calendarId").getValue<String>() ?: "",
                semStart = semConfig.child("start").getValue<String>() ?: "2025-12-01",
                semEnd = semConfig.child("end").getValue<String>() ?: "2026-05-31"
            )
        } catch (e: Exception) {
            Log.e(TAG, "Failed to get calendar config: ${e.message}")
            null
        }
    }
    
    /**
     * Run Google Calendar sync - matches web app's runGlobalCalendarSync
     * 1. Fetch config from Firebase
     * 2. Fetch events from Google Calendar API
     * 3. Compare with existing Firebase events
     * 4. Push to Firebase if different
     */
    suspend fun runGoogleCalendarSync(batch: String): Boolean {
        Log.d(TAG, "Starting Google Calendar sync for batch: $batch")
        
        try {
            // 1. Get calendar config
            val config = getCalendarConfig(batch)
            if (config == null || config.apiKey.isEmpty() || config.calendarId.isEmpty()) {
                Log.w(TAG, "Calendar sync: Missing config for batch $batch")
                return false
            }
            
            // 2. Fetch events from Google Calendar API
            val events = googleCalendarService.fetchEvents(
                apiKey = config.apiKey,
                calendarId = config.calendarId,
                startDate = config.semStart,
                endDate = config.semEnd
            )
            
            if (events.isEmpty()) {
                Log.d(TAG, "Calendar sync: No events fetched from Google")
                return false
            }
            
            // 3. Get existing Firebase events for comparison
            val existingRef = database.getReference("calendars/$batch/events")
            val existingSnapshot = awaitSingleValue(existingRef)
            val existingEvents = existingSnapshot.children.mapNotNull { child ->
                try {
                    CalendarEvent(
                        id = child.key ?: "",
                        title = child.child("title").getValue<String>() ?: "",
                        date = child.child("date").getValue<String>() ?: "",
                        groupId = child.child("groupId").getValue<String>(),
                        type = "",
                        startTime = null,
                        endTime = null,
            ...
            // 4. Compare using hash (matching web app's getQuickHash)
            val newHash = googleCalendarService.getEventsHash(events)
            val existingHash = googleCalendarService.getEventsHash(existingEvents)
            
            if (newHash == existingHash) {
                Log.d(TAG, "Calendar sync: No changes detected, skipping push")
                return true
            }
            
            // 5. Push to Firebase
            Log.d(TAG, "Calendar sync: Changes detected, pushing ${events.size} events to Firebase")
            pushCalendarEventsToFirebase(batch, events)
            
            // 6. Update local DB immediately
            val entities = events.map { CalendarEventEntity.fromCalendarEvent(it) }
            localDb.calendarEventDao().replaceEvents(entities)
    ...
    /**
     * Push calendar events to Firebase
     */
    private suspend fun pushCalendarEventsToFirebase(batch: String, events: List<CalendarEvent>) {
        try {
            val ref = database.getReference("calendars/$batch/events")
            ...
            val eventsList = events.mapIndexed { index, event ->
                mapOf(
                    ...
                    "fullTime" to (event.fullTime ?: "All Day")
                )
            }
            
            ref.setValue(eventsList)
            Log.d(TAG, "Pushed ${events.size} events to Firebase")
        } catch (e: Exception) {
            Log.e(TAG, "Failed to push events to Firebase: ${e.message}")
        }
    }
}
