package com.elvan.rmdneram.ui.theme

import androidx.compose.ui.text.font.Font
import androidx.compose.ui.text.font.FontFamily
import androidx.compose.ui.text.font.FontWeight
import com.elvan.rmdneram.R

/**
 * Mukta Malar Tamil Font Family
 * A Unicode-compliant Tamil typeface from Google Fonts
 */
val MuktaMalarFontFamily = FontFamily(
    Font(R.font.mukta_malar_light, FontWeight.Light),
    Font(R.font.mukta_malar_regular, FontWeight.Normal),
    Font(R.font.mukta_malar_medium, FontWeight.Medium),
    Font(R.font.mukta_malar_semibold, FontWeight.SemiBold),
    Font(R.font.mukta_malar_bold, FontWeight.Bold)
)
